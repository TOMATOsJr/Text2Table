...
| **4. KG → Table** | Normalize relations, deduplicate, assemble table |
...
3. Remove citation markers — `[1]`, `[citation needed]`, etc.
...
**Entity resolution step** (the key addition):
...
Discriminative pipeline — **three models**, each handling a different subtask:
...
- **Step 1** identifies *where* attribute values appear in the sentence
...
# Knowledge Graph → Table Conversion
...
**Gold standard:** Constructed using WikiBio test set — **36,415** records, **140,249** triples
...
| **Multi-sentence coreference** | Pronoun → title heuristic misses family member references |
...
**Future Work:**
- Better coreference resolution (beyond pronoun → title heuristic)
- Cross-domain generalization beyond biographies
- End-to-end joint training (text → table without separate stages)
