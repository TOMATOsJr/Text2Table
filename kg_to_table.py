...
def clean_value(text: str) -> str:
    """Normalize text by removing tokenization noise and fixing whitespace."""
    if not text:
        return ""
    
    # 1. Handle NLTK noise tokens
    text = text.replace("-lrb-", "(").replace("-rrb-", ")")
    text = text.replace("-lsb-", "[").replace("-rsb-", "]")
    text = text.replace("``", '"').replace("''", '"')
    
    # 2. Fix punctuation spacing (e.g., "London , England" -> "London, England")
    import re
...
def refine_date_value(text: str) -> str:
    """Isolate a single date/year string from potentially smushed values."""
    if not text:
        return ""
    
    # Match common date patterns
    import re
    # Match variations: "12 February 1949", "February 12, 1949", "1949-02-12"
    pattern = r'(\b\d{1,2}\s+[a-zA-Z]+\s+\d{4}\b|\b[a-zA-Z]+\s+\d{1,2},?\s+\d{4}\b|\b\d{4}-\d{2}-\d{2}\b)'
    dates = re.findall(pattern, text)
    
    years = re.findall(r'\b\d{4}\b', text)
    
    # Priority: Full Date > Year
    best_match = None
    if dates:
        best_match = dates[0]
    elif years:
    ...
    if best_match and parser:
        try:
            # Reformat to match WikiBio style: "12 february 1949"
            dt = parser.parse(best_match)
            return dt.strftime("%d %B %Y").lower().lstrip("0")
        except:
            pass
            
    return best_match if best_match else text  # Fallback to original cleaned text
...
def load_and_group(input_path: Path) -> dict[Any, dict]:
    """Load KG and group into per-entity records (Unified Schema Support).
    
    Supports:
    1. Legacy Stage 5 format: list of objects with "record_id" and "triples[]".
    2. Consolidated format: dict with "nodes" and "edges" lists.
    """
    ...
    # Case A: Consolidated Global KG (dict with "nodes" and "edges")
    if isinstance(raw_data, dict) and "edges" in raw_data:
        print(f"  Detected Consolidated KG format ({len(raw_data.get('edges', [])):,} edges)")
        # We treat each unique source as a record
        for edge in raw_data["edges"]:
            ...
            if subj not in records:
                records[subj] = {
                    "title": subj,
                    "triples": [],
                    "entities": [], # We don't have per-entity types here yet
                    "sentences": [],
                }
            # Convert edge to triple format expected by downstream logic
            records[subj]["triples"].append({
                "subject": subj,
    ...
    # Case B: Legacy Per-Sentence List
    print(f"  Detected Legacy Record list format ({len(raw_data):,} entries)")
    skipped = 0
    for entry in raw_data:
        ...
        rid = entry.get("record_id", title) # fallback to title if ID missing
...
def normalise_triples(
    ...
    for triple in triples:
        # Preferred: Use QKV Canonicalization if available and valid
        relation = triple.get("canonical_relation") or triple.get("relation", "")
        if relation == "unknown": # QKV confidence threshold not met
             relation = triple.get("relation", "")
        
        relation = relation.strip().lower()
        
        # Prefer canonicalized object/subject if available
        value = triple.get("canonical_object") or triple.get("object", "")
        value = value.strip()
...
def aggregate_attributes(
    ...
    for attr, val in attr_value_pairs:
        # Skip redundant name attribute (per user request: "keep title remove name")
        if attr == "name":
            continue
        ...
        # Specialized cleaning for dates to avoid "date bleeding"
        if attr in ("birth_date", "death_date", "debutteam_date"):
            c_val = refine_date_value(c_val)

        grouped[attr].append(c_val)
...
def write_csv(tables: dict[int, dict], output_path: Path) -> None:
    """Write biographical tables in a 'stacked' Key/Value CSV format.

    The format consists of a single header 'Key,Value' followed by records.
    Each record begins with a 'Title' row and is separated by a blank line.
    """
    output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    with output_path.open("w", encoding="utf-8") as f:
        ...
        for rid in sorted(tables):
            tbl = tables[rid]
            # Write Title row
            title = tbl['title'].replace('"', '""')
            f.write(f'Title,"{title}"\n')

            # Write attribute rows
            for attr, values in tbl["table"]:
                val_str = " ; ".join(values).replace('"', '""')
                f.write(f'{attr},"{val_str}"\n')

            # Blank line between records
            f.write("\n")

    print(f"  → Wrote {len(tables)} tables in CSV format to {output_path}")
...
def main() -> None:
    ...
    ap.add_argument(
        "--output-format", choices=["jsonl", "box", "markdown", "csv"], default="jsonl",
