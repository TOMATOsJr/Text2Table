...
def parse_table_blocks(path: Path) -> Tuple[Dict[str, List[Tuple[str, str]]], List[Triple]]:
    ...
    for idx, row in enumerate(tqdm(rows, total=len(rows), desc=f"parse:{path.name}", unit="row")):
        # Skip header if it exists
        if idx == 0 and len(row) >= 2 and row[0].lower() == "key" and row[1].lower() == "value":
            continue
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--pred_csv",
        type=Path,
        default=Path("report_assets/test_1k_tables_hybrid.csv"),
        help="Predicted table CSV path.",
    )
    parser.add_argument(
        "--gold_csv",
        type=Path,
        default=Path("gold/gold_test_table_dual.csv"),
    ...
    parser.add_argument(
        "--output_json",
        type=Path,
        default=Path("report_assets/test_1k_eval_results_hybrid.json"),
...
def main() -> None:
    ...
    # Filter triples to common titles for fair Recall/F1 during subset evaluation
    c_set = set(common_titles)
    pred_subset = [t for t in pred_triples if normalize_text(t.title) in c_set]
    gold_subset = [t for t in gold_triples if normalize_text(t.title) in c_set]

    triple_metrics = fuzzy_triple_prf(pred_subset, gold_subset, threshold=args.fuzzy_threshold)
    ...
    result = {
        ...
        "num_pred_triples": len(pred_subset),
        "num_gold_triples_total": len(gold_triples),
        "num_gold_triples_subset": len(gold_subset),
        "chrf": chrf,
